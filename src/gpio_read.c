...
const unsigned short signal_gpio = 4;
...
short readData()
{
	...
	while (1)
	{
		...
		while (digitalRead(signal_gpio) == HIGH)
		{
			signal_length++;
...
void *threadFunc(void* arg)
{
       ...
       for (;;)
	{
		...
		pinMode(signal_gpio, INPUT);		// 'INPUT' equals 'HIGH' level. And signal read mode
